...
use parachain_template_runtime::{
	AccountId, AllPalletsWithSystem, Balance, Balances, Executive, ParachainSystem, Runtime,
	RuntimeCall, RuntimeOrigin, Timestamp, SLOT_DURATION,
};
...
fn initialize_block(block: u32, prev_header: &Option<Header>) {
	...
	let pre_digest = Digest {
		logs: vec![DigestItem::PreRuntime(AURA_ENGINE_ID, Slot::from(u64::from(block)).encode())],
	};
